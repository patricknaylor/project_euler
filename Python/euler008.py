#########################################################
#
# Project Euler Problem 8
#
# The four adjacent digits in the 1000-digit number below that have the greatest product are 9 × 9 × 8 × 9 = 5832.
# Find the thirteen adjacent digits in the 1000-digit number that have the greatest product.
# What is the value of this product?
#
#########################################################

# here is the 1000-digit number, saved as a string
big_number = ''


# input a string and output the product of the digits; empty string gets assigned 1
def prod(string):
    product = 1
    for i in string:
        product = product * int(i)
    return product       


# just check each substring in a brute force way
# this could be done more efficiently; we only really need to know the difference in the ends of a
# substring to compare products
def greatest_prod(big_num, increment=13):
    max_product = 0
    max_index = 0
    
    for index in range(len(big_num) - increment - 1):
        new_product = prod(big_num[index: index + increment])
        if new_product > max_product:
            max_product = new_product
            max_index = index

    return max_product, max_index


greatest_product, location = greatest_prod(big_number)

print("The greatest product of {} adjacent digits is {}; the digits are {}.".format(13, greatest_product, big_number[location:location + 13]))
