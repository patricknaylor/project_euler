...
# remember: leap years are the ones divisible by 4; unless they're divisible by 100;
# but they are leap years if divisible by 400
...
def next_day(date):
    ...
    # this is the easy one, since this always happens
    week = (week + 1) % 7

    is_leap_year = check_leap_year(date)

    # this is the next easiest-- if its dec 31, then the next day is the first day of the next year
...
def count_sundays():
    ...
    while date[0] < 2001:
        if 1900 < date[0] < 2001 and (date[2], date[3]) == (1, 6):
            count += 1
        date = next_day(date)
...
print("There are {} Sundays on the first of a month in the twentieth century.".format(sundays))
